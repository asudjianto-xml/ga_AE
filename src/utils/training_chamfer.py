"""Training utilities for VAE with Tangent Chamfer Loss"""
import torch
...
class ChamferTrainer:
    """
    Trainer specifically for VAE_TangentChamfer models.

    Handles:
    - Reference bank updates
    - Chamfer loss computation
    - Standard VAE diagnostics
    """
    ...
    def __init__(
        ...
        self.pca_directions = compute_data_pca_directions(
            self.train_data_torch[:5000],
            k_max=8
        )

        # Logging
        self.metrics_log = []
        self.model_type = config.get('model_type', 'vae_chamfer')

        # Bank update frequency
        self.bank_update_every = config.get('bank_update_every', 1)
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            # Update reference bank periodically
            if batch_idx % self.bank_update_every == 0:
                self.model.update_reference_bank(x)
    ...
    def train(self):
        ...
        # Initialize reference bank with a batch from training data
        print("Initializing reference bank...")
        init_batch = next(iter(self.train_loader))[0].to(self.config['device'])
        self.model.update_reference_bank(init_batch)
        print(f"Reference bank initialized with {init_batch.size(0)} samples")
        ...
        for epoch in range(self.config['epochs']):
            ...
            if epoch % 10 == 0:
                print(f"Epoch {epoch}/{self.config['epochs']}: "
                      ...
                      f"Chamfer = {train_metrics.get('chamfer_loss', 0):.4f}")
